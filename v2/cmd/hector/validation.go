...
// Ported from legacy pkg/cli/mutual_exclusivity.go with v2-specific flags.
//
// V2 adaptations:
// - Added --storage, --storage-db (v2 storage options)
// - Added --rag-watch, --mcp-parser-tool (v2 RAG options)
// - Removed --vectordb (v2 uses different approach)
...
// Ported line-by-line from legacy pkg/cli/mutual_exclusivity.go.
// V2 adaptation: Updated examples to use v2 command structure.
...
// V2 adaptation: Updated skip commands for v2 command structure.
...
// V2 adaptation: Updated error message patterns for v2's error format.
func FormatConfigError(configPath string, err error) string {
	...
	// V2 adaptation: v2 uses different error format than legacy's ProcessConfigPipeline
	...
	// For reference errors (v2 specific)
