...
func (c *ValidateCmd) Run(cli *CLI) error {
	...
	// V2 adaptation: Use config.LoadDotEnvForConfig
	config.LoadDotEnvForConfig(c.Config)

	// Load configuration using v2's config loader
	// Legacy used config.LoadConfig with LoaderOptions
	// V2 adaptation: Use config.LoadConfigFile which handles loading and validation
	...
	// V2 note: config.LoadConfigFile already calls SetDefaults() and Validate()
	// Legacy used ProcessConfigPipeline for this, but v2's loader handles it internally
...
// Ported line-by-line from legacy.
// Note: In v2, processing errors are typically caught during LoadConfigFile,
