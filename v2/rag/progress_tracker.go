...
func (pt *ProgressTracker) printProgress() {
	...
	if eta > 0 {
		output += fmt.Sprintf(" | ETA: %s", formatDurationForProgress(eta))
	}
...
func (pt *ProgressTracker) printFinalSummary() {
	...
	elapsed := stats.ElapsedTime
	fmt.Printf("   Time:    %s\n", formatDurationForProgress(elapsed))
...
// formatDurationForProgress formats a duration in a human-readable way.
