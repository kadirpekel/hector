...
// sessionWrapper adapts session.Session to agent.Session for indexing.
type sessionWrapper struct {
	sess   session.Session
	events *eventList
}

func (w *sessionWrapper) ID() string                { return w.sess.ID() }
func (w *sessionWrapper) AppName() string           { return w.sess.AppName() }
func (w *sessionWrapper) UserID() string            { return w.sess.UserID() }
func (w *sessionWrapper) State() agent.State        { return w.sess.State() }
func (w *sessionWrapper) Events() agent.Events      { return w.events }
func (w *sessionWrapper) LastUpdateTime() time.Time { return w.sess.LastUpdateTime() }

type eventList struct {
	events []*agent.Event
}

func (e *eventList) All() iter.Seq[*agent.Event] {
...
func (e *eventList) At(i int) *agent.Event {
