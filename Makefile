...
.PHONY: help build build-release build-headless build-studio build-unrestricted install test clean fmt vet lint release version test-coverage test-coverage-summary test-package test-race test-verbose dev ci install-lint quality pre-commit
...
help:
	...
	@echo "  build-release      - Build the hector binary (production, with Studio UI)"
	@echo "  build-headless     - Build without UI (API-only, smaller binary)"
...
build:
	...
	CGO_ENABLED=0 go build -ldflags "$(LDFLAGS_VERSION)" -o hector ./cmd/hector
	@ls -lh hector

# Build the binary (production, stripped, with Studio UI embedded)
STUDIO_DIR ?= ./studio
build-release:
	@echo "Building hector (production)..."
	@if ! command -v npm >/dev/null 2>&1; then \
		echo "Error: npm is required for build-release. Install Node.js or use 'make build' for a headless binary."; \
		exit 1; \
	fi
	@if [ ! -d "$(STUDIO_DIR)" ]; then \
		echo "Error: Studio directory not found at $(STUDIO_DIR). Ensure you have a complete checkout of the repository."; \
		exit 1; \
	fi
	...
	CGO_ENABLED=0 go build -ldflags "$(LDFLAGS_RELEASE)" -o hector ./cmd/hector
	@ls -lh hector

# Build without UI (API-only, smaller binary)
build-headless:
	@echo "Building hector (headless - no UI)..."
	CGO_ENABLED=0 go build -ldflags "$(LDFLAGS_RELEASE)" -o hector ./cmd/hector
	@ls -lh hector
...
build-unrestricted:
	...
	@echo ""
	CGO_ENABLED=0 go build -tags=unrestricted -ldflags "$(LDFLAGS_RELEASE)" -o hector-unrestricted ./cmd/hector
...
version:
	...
	@go run -ldflags "-X 'github.com/verikod/hector.Version=$(VERSION)' -X 'github.com/verikod/hector.BuildDate=$(shell date -u +%Y-%m-%dT%H:%M:%SZ)' -X 'github.com/verikod/hector.GitCommit=$(shell git rev-parse --short HEAD 2>/dev/null || echo unknown)'" ./cmd/hector version
