...
func (cm *IndexCheckpointManager) ShouldProcessFile(path string, size int64, modTime time.Time) bool {
	...
	if !exists {
		slog.Debug("Checkpoint: file not found, will process",
			"path", path,
			"checkpoint_files_count", len(cm.checkpoint.ProcessedFiles))
	...
	if currentHash != fileCheckpoint.Hash {
		slog.Debug("Checkpoint: file hash changed, will re-process",
			"path", path,
			"current_hash", currentHash,
			"checkpoint_hash", fileCheckpoint.Hash,
			"current_size", size,
			"checkpoint_size", fileCheckpoint.Size,
			"current_mod", modTime.Unix(),
			"checkpoint_mod", fileCheckpoint.ModTime.Unix())
