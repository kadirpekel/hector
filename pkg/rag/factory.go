...
func NewDataSourceFromConfig(cfg *config.DocumentSourceConfig, deps *FactoryDeps) (DataSource, error) {
	...
	case "blob":
		if cfg.Blob == nil {
			return nil, fmt.Errorf("blob config is required for blob source")
		}

		blobCfg := BlobSourceConfig{
			URL:         cfg.Blob.URL,
		...
		// Apply defaults
		if blobCfg.MaxFileSize == 0 {
			blobCfg.MaxFileSize = 10 * 1024 * 1024 // 10MB
		}
		if blobCfg.Exclude == nil {
			blobCfg.Exclude = []string{".*", "node_modules", "__pycache__", "vendor", ".git"}
		}

		// Use background context for bucket initialization
		return NewBlobSourceFromConfig(context.Background(), blobCfg)
...
func newSQLSourceFromConfig(cfg *config.SQLSourceConfig, deps *FactoryDeps) (*SQLSource, error) {
	...
	if deps.DatabaseDSN == "" {
		return nil, fmt.Errorf("DatabaseDSN is required for SQL source")
	}

	// Parse database config from DSN
	dbCfg, err := config.ParseDSN(deps.DatabaseDSN)
	if err != nil {
...
func NewDocumentStoreFromConfig(
	...
	if storeCfg.MCPParsers != nil && len(storeCfg.MCPParsers.ToolNames) > 0 {
		...
		localBasePath := ""
		if storeCfg.Source != nil && storeCfg.Source.Type == "blob" && storeCfg.Source.Blob != nil {
			// For file:// URLs, extract the local path
			if strings.HasPrefix(storeCfg.Source.Blob.URL, "file://") {
				localBasePath = strings.TrimPrefix(storeCfg.Source.Blob.URL, "file://")
			}
		}
...
type DBPoolAdapter struct {
	pool      *config.DBPool
	parsedDSN *config.DatabaseConfig
}
...
func NewDBPoolAdapter(pool *config.DBPool, databaseDSN string) *DBPoolAdapter {
	parsedDSN, err := config.ParseDSN(databaseDSN)
	if err != nil {
		// Return adapter with nil parsedDSN - will cause errors if database operations are attempted
		return &DBPoolAdapter{pool: pool, parsedDSN: nil}
	}
	return &DBPoolAdapter{pool: pool, parsedDSN: parsedDSN}
}

// Get returns a database connection using the primary database.
func (a *DBPoolAdapter) Get(name string) (*sql.DB, string, error) {
	if a.parsedDSN == nil {
		return nil, "", fmt.Errorf("database config not parsed")
	}
	conn, err := a.pool.Get(a.parsedDSN)
	if err != nil {
		return nil, "", err
	}
	return conn, a.parsedDSN.Driver, nil
}
