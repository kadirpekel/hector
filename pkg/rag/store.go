...
func NewDocumentStore(cfg DocumentStoreConfig) (*DocumentStore, error) {
	...
	checkpointMgr := NewIndexCheckpointManager(collection, sourcePath, enableCheckpoints)
...
func (s *DocumentStore) Index(ctx context.Context) error {
	...
	if s.source.Type() == "directory" {
		...
		if checkpoint != nil && err == nil {
			...
			if processedCount > 0 {
				...
				// Sanity check: Validate that the checkpoint is consistent with the vector store.
				// This handles cases where the vector store is ephemeral (e.g. in-memory chromem)
				// but the checkpoint is persistent (disk). On restart, store is empty but checkpoint is full.
				needsInvalidation := false
				for _, f := range checkpoint.ProcessedFiles {
					if f.Status == "indexed" {
						// Check if this document exists in the store
						// We don't care about the content, just presence
						doc, err := s.GetDocument(ctx, f.Path)
						if err != nil || doc == nil {
							slog.Warn("Checkpoint inconsistency detected: document listed as indexed is missing from store. Invalidating checkpoint.",
								"document", f.Path,
								"store_name", s.name)
							needsInvalidation = true
						}
						// Checking one is sufficient to detect a store wipe
						break
					}
				}

				if needsInvalidation {
					if err := s.checkpointManager.ClearCheckpoint(); err != nil {
						slog.Warn("Failed to clear invalid checkpoint", "error", err)
					}
					checkpoint = nil
				}
			}
	...
	defer func() {
		...
		// Save final checkpoint (force save to bypass throttle)
		if s.source.Type() == "directory" {
			_ = s.checkpointManager.ForceSave()
		}
	}()
	...
	for {
		...
		case doc, ok := <-docChan:
			...
			modTime := time.Time{}
			fileSize := doc.Size // Use doc.Size directly (not from metadata)
...
func (s *DocumentStore) handleEvent(ctx context.Context, event DocumentEvent) {
	...
	case DocumentEventCreate, DocumentEventUpdate:
		...
		slog.Info("Indexed document on change",
			"document", doc.ID,
			"event", event.Type)
	...
	case DocumentEventDelete:
		...
		slog.Info("Deleted document on change", "document", event.Document.ID)
...
func (s *DocumentStore) Stats() DocumentStoreStats {
	...
	// Get actual count from vector provider (handles persistent data)
	indexedCount := len(s.indexedDocs) // Fallback to in-memory
	if s.engine != nil && s.engine.provider != nil {
		ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
		defer cancel()
		if count, err := s.engine.provider.Count(ctx, s.collection); err == nil && count > 0 {
			indexedCount = count
		}
	}
