...
// NewAuthValidator has been removed.
// Authentication is a server-level concern, not a runtime concern.
// Use auth.NewValidatorFromConfig(serverConfig.Auth) at the server layer instead.
...
func (a *toolAdapter) Execute(ctx context.Context, args map[string]any) (rag.ToolResult, error) {
	...
	// Call the tool
	// Note: MCP tools don't require a full tool.Context for execution
