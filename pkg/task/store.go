...
type taskStoreRow struct {
	ID            string
	AppName       string // Multi-app tenant isolation
	ContextID     string
	StatusJSON    string
	HistoryJSON   string
...
const (
	...
	// Note: app_name provides multi-app tenant isolation.
...
CREATE INDEX IF NOT EXISTS idx_a2a_tasks_context_id ON a2a_tasks(app_name, context_id)`
...
func (s *SQLTaskStore) Save(ctx context.Context, task *a2a.Task) error {
	...
	// Get app_name from context for multi-tenant isolation
	...
	if task.Metadata != nil {
		if expectedUpdatedAt, ok := task.Metadata["_updated_at"].(string); ok {
			currentUpdatedAt, err := s.getTaskUpdatedAt(ctx, appName, task.ID)
	...
	query := `
INSERT INTO a2a_tasks (id, app_name, context_id, status_json, history_json, artifacts_json, metadata_json, created_at, updated_at)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
...
ON CONFLICT (app_name, id) DO UPDATE SET
...
`
	...
	args := []interface{}{
		row.ID, row.AppName, row.ContextID, row.StatusJSON,
...
func (s *SQLTaskStore) Get(ctx context.Context, taskID a2a.TaskID) (*a2a.Task, error) {
	appName := app.IDFromContext(ctx)
	slog.Debug("TaskStore.Get called", "taskID", taskID, "appName", appName)

	query := `
SELECT id, app_name, context_id, status_json, history_json, artifacts_json, metadata_json, created_at, updated_at
FROM a2a_tasks
...
`
	...
	err := s.db.QueryRowContext(ctx, query, appName, string(taskID)).Scan(
		&row.ID, &row.AppName, &row.ContextID, &row.StatusJSON,
		&row.HistoryJSON, &row.ArtifactsJSON, &row.MetadataJSON,
		&row.CreatedAt, &row.UpdatedAt,
	)
	if err == sql.ErrNoRows {
		slog.Debug("TaskStore.Get: task not found", "taskID", taskID, "appName", appName)
		return nil, a2a.ErrTaskNotFound
	}
	if err != nil {
		slog.Error("TaskStore.Get: query error", "taskID", taskID, "appName", appName, "error", err)
		return nil, fmt.Errorf("failed to query task: %w", err)
	}

	slog.Debug("TaskStore.Get: found task", "taskID", taskID, "appName", row.AppName, "contextID", row.ContextID)
...
func (s *SQLTaskStore) getTaskUpdatedAt(ctx context.Context, appName string, taskID a2a.TaskID) (string, error) {
	query := `SELECT updated_at FROM a2a_tasks WHERE app_name = ? AND id = ?`
	if s.dialect == "postgres" || s.dialect == "pgx" {
		query = `SELECT updated_at FROM a2a_tasks WHERE app_name = $1 AND id = $2`
	}

	var updatedAt time.Time
	err := s.db.QueryRowContext(ctx, query, appName, string(taskID)).Scan(&updatedAt)
	if err != nil {
		return "", err
	}
...
func (s *SQLTaskStore) taskToRow(ctx context.Context, task *a2a.Task) (*taskStoreRow, error) {
	now := time.Now()
	appName := app.IDFromContext(ctx)
...
func (s *SQLTaskStore) List(ctx context.Context, appName, contextID string) ([]*a2a.Task, error) {
	...
	if s.dialect == "postgres" || s.dialect == "pgx" {
		...
		SELECT id, app_name, context_id, status_json, history_json, artifacts_json, metadata_json, created_at, updated_at
		FROM a2a_tasks
		WHERE app_name = $1 AND context_id = $2
		ORDER BY created_at DESC
		`
	}

	rows, err := s.db.QueryContext(ctx, query, appName, contextID)
	if err != nil {
	...
	for rows.Next() {
		...
		if err != nil {
			slog.Warn("Failed to deserialize task", "id", row.ID, "error", err)
			continue
		}
...
// This is effectively an alias for Save but semantically useful for the interface.
...
func (s *SQLTaskStore) DeleteByApp(ctx context.Context, appName string) error {
	query := `DELETE FROM a2a_tasks WHERE app_name = ?`
	if s.dialect == "postgres" || s.dialect == "pgx" {
		query = `DELETE FROM a2a_tasks WHERE app_name = $1`
	}

	_, err := s.db.ExecContext(ctx, query, appName)
	if err != nil {
		return fmt.Errorf("failed to delete tasks for app %s: %w", appName, err)
	}
