...
type Service interface {
	...
	// This is used by summary_buffer strategy to compact the session after summarization.
	ReplaceEvents(ctx context.Context, session Session, events []*agent.Event) error
...
type GetRequest struct {
	...
	// AfterEventID returns events after the event with this ID (by sequence_num).
	// Optional: if empty, returns all events.
	AfterEventID string
}
...
func (s *memorySession) replaceEvents(events []*agent.Event) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (e *memoryEvents) replace(events []*agent.Event) {
	e.mu.Lock()
	defer e.mu.Unlock()
	e.events = events
}
...
func (s *inMemoryService) ReplaceEvents(ctx context.Context, session Session, events []*agent.Event) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *inMemoryService) DeleteByApp(ctx context.Context, appName string) error {
	s.mu.Lock()
	defer s.mu.Unlock()

	// Inefficient but simpler: iterate and delete
	// Since keys are appName:userID:sessionID, we can check prefix
	prefix := appName + ":"
