...
func (s *SQLSessionService) Get(ctx context.Context, req *GetRequest) (*GetResponse, error) {
	...
	events, err := s.getEventsFiltered(ctx, req.AppName, req.UserID, req.SessionID, req.NumRecentEvents, req.AfterEventID)
	if err != nil {
...
// ReplaceEvents replaces all events in the session with the given events.
// This is used by the summary_buffer strategy to compact the session after summarization.
// The operation is atomic - either all events are replaced or none are.
func (s *SQLSessionService) ReplaceEvents(ctx context.Context, session Session, events []*agent.Event) error {
	if session == nil {
		return fmt.Errorf("session is nil")
	}
	...
	// Delete all existing events for this session
	deleteQuery := `DELETE FROM session_events WHERE app_name = ? AND user_id = ? AND session_id = ?`
	if s.dialect == "postgres" {
		deleteQuery = convertToPostgresPlaceholders(deleteQuery)
	}
	if _, err := tx.ExecContext(ctx, deleteQuery, session.AppName(), session.UserID(), session.ID()); err != nil {
		return fmt.Errorf("failed to delete existing events: %w", err)
	}

	// Insert new events with sequential sequence numbers
	for i, event := range events {
		if event == nil || event.Partial {
			continue
		}
		if err := s.insertEventTx(ctx, tx, session, event, i+1); err != nil {
			return fmt.Errorf("failed to insert event: %w", err)
		}
	}
...
// DeleteByApp removes all sessions and associated data for a specific app.
func (s *SQLSessionService) DeleteByApp(ctx context.Context, appName string) error {
	...
	// 1. Delete events
	eventQuery := `DELETE FROM session_events WHERE app_name = ?`
	if s.dialect == "postgres" {
		eventQuery = convertToPostgresPlaceholders(eventQuery)
	}
	if _, err := tx.ExecContext(ctx, eventQuery, appName); err != nil {
		return fmt.Errorf("failed to delete events: %w", err)
	}

	// 2. Delete sessions
	sessionQuery := `DELETE FROM sessions WHERE app_name = ?`
	if s.dialect == "postgres" {
		sessionQuery = convertToPostgresPlaceholders(sessionQuery)
	}
	if _, err := tx.ExecContext(ctx, sessionQuery, appName); err != nil {
		return fmt.Errorf("failed to delete sessions: %w", err)
	}

	// 3. Delete user states
	userStateQuery := `DELETE FROM user_states WHERE app_name = ?`
	if s.dialect == "postgres" {
		userStateQuery = convertToPostgresPlaceholders(userStateQuery)
	}
	if _, err := tx.ExecContext(ctx, userStateQuery, appName); err != nil {
		return fmt.Errorf("failed to delete user states: %w", err)
	}

	// 4. Delete app states
	appStateQuery := `DELETE FROM app_states WHERE app_name = ?`
	if s.dialect == "postgres" {
		appStateQuery = convertToPostgresPlaceholders(appStateQuery)
	}
	if _, err := tx.ExecContext(ctx, appStateQuery, appName); err != nil {
		return fmt.Errorf("failed to delete app states: %w", err)
	}
...
func (s *SQLSessionService) getEvents(ctx context.Context, appName, userID, sessionID string) ([]*agent.Event, error) {
	return s.getEventsFiltered(ctx, appName, userID, sessionID, 0, "")
}

func (s *SQLSessionService) getEventsFiltered(ctx context.Context, appName, userID, sessionID string, numRecent int, afterEventID string) ([]*agent.Event, error) {
	...
	if numRecent > 0 {
		...
		if afterEventID != "" {
			query += " AND sequence_num > COALESCE((SELECT sequence_num FROM session_events WHERE app_name = ? AND user_id = ? AND session_id = ? AND id = ?), 999999999)"
			args = append(args, appName, userID, sessionID, afterEventID)
		}
...
func eventToRow(session Session, event *agent.Event, seqNum int) (*eventRow, error) {
	...
	// Artifact content
	if event.Artifact != nil && len(event.Artifact.Parts) > 0 {
		// Derive role from author
		if event.Author == agent.AuthorUser {
			row.Role = string(a2a.MessageRoleUser)
		} else {
			row.Role = string(a2a.MessageRoleAgent)
		}
		partsJSON, err := json.Marshal(event.Artifact.Parts)
		if err != nil {
			return nil, fmt.Errorf("failed to marshal artifact parts: %w", err)
		}
	...
	// Rich content - use accessor methods for metadata-stored fields
...
func rowToEvent(row *eventRow) (*agent.Event, error) {
	...
	if row.ContentJSON != "" {
		...
		if len(parts) > 0 {
			event.Artifact = &a2a.Artifact{Parts: parts}
		}
	}
	...
	// Rich content - use setter methods for metadata-stored fields
	if row.ThinkingJSON != "" {
