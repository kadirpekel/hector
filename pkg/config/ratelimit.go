...
// Rate limiting uses the primary database (cfg.Database) for persistence.
...
func (c *RateLimitConfig) SetDefaults() {
	...
	// Set default IP headers if not specified
	if len(c.IPHeaders) == 0 {
		c.IPHeaders = []string{"X-Forwarded-For", "X-Real-IP"}
	}
}
