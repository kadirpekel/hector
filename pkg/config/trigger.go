...
type WebhookInputConfig struct {
	...
	SessionID string `yaml:"session_id,omitempty" json:"session_id,omitempty" jsonschema:"title=Session ID Template,description=Template for deriving session ID from payload for conversation continuity"`
...
type TriggerConfig struct {
	...
	// Default: schedule
	Type TriggerType `yaml:"type,omitempty" json:"type,omitempty" jsonschema:"title=Trigger Type,description=Type of trigger,enum=schedule,enum=webhook,default=schedule"`
...
func (c *TriggerConfig) SetDefaults() {
	// Default trigger type to schedule
	if c.Type == "" {
		c.Type = TriggerTypeSchedule
	}
