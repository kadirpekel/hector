...
type NotificationConfig struct {
	...
	// Default: webhook
	Type NotificationType `yaml:"type,omitempty" json:"type,omitempty" jsonschema:"title=Type,description=Notification type,enum=webhook,default=webhook"`
...
func (c *NotificationConfig) SetDefaults() {
	...
	// Default to task.completed event
	if len(c.Events) == 0 {
		c.Events = []NotificationEvent{NotificationEventTaskCompleted}
	}
	if c.Retry == nil {
