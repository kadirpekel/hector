...
func (c *LLMConfig) Validate() error {
	validProviders := map[LLMProvider]bool{
		...
		LLMProviderMistral:   true,
		LLMProviderCohere:    true,
	}

	if c.Provider != "" && !validProviders[c.Provider] {
		return fmt.Errorf("invalid provider %q (valid: anthropic, openai, gemini, ollama, deepseek, groq, mistral, cohere)", c.Provider)
	}

	// Ollama doesn't require API key
	if c.Provider != LLMProviderOllama && c.APIKey == "" {
		envVar := getAPIKeyEnvVarName(c.Provider)
		return fmt.Errorf("api_key is required for provider %q (set %s environment variable or provide api_key in config)", c.Provider, envVar)
	}
...
// getAPIKeyEnvVarName returns the environment variable name for a provider's API key.
func getAPIKeyEnvVarName(provider LLMProvider) string {
	switch provider {
	case LLMProviderAnthropic:
