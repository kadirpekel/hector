...
type AgentConfig struct {
	...
	DisableSafetyProtocols bool `yaml:"disable_safety_protocols,omitempty" json:"disable_safety_protocols,omitempty" jsonschema:"title=Disable Safety Protocols,description=Disable automatic injection of tool safety and CoT protocols,default=false"`
	...
	Type string `yaml:"type,omitempty" json:"type,omitempty" jsonschema:"title=Agent Type,description=Type of agent,enum=llm,enum=sequential,enum=parallel,enum=loop,enum=remote,enum=runner,enum=conditional,default=llm"`
	...
	// === Conditional Agent Configuration (Type="conditional") ===
	...
	ConditionAgent string `yaml:"condition_agent,omitempty" json:"condition_agent,omitempty" jsonschema:"title=Condition Agent,description=Agent that evaluates the routing condition"`
	...
	// Default: "safe"
	// Only used when Type="conditional".
	ConditionField string `yaml:"condition_field,omitempty" json:"condition_field,omitempty" jsonschema:"title=Condition Field,description=JSON field to check in condition agent output,default=safe"`

	// OnTrueAgent references the agent to run when condition is true.
	// Only used when Type="conditional".
	OnTrueAgent string `yaml:"on_true_agent,omitempty" json:"on_true_agent,omitempty" jsonschema:"title=On True Agent,description=Agent to run when condition is true"`

	// OnFalseResponse is the static response when condition is false.
	// Only used when Type="conditional".
	OnFalseResponse string `yaml:"on_false_response,omitempty" json:"on_false_response,omitempty" jsonschema:"title=On False Response,description=Static response when condition is false,default=I cannot process this request."`
...
type ContextConfig struct {
	...
	// Default: 20
	WindowSize int `yaml:"window_size,omitempty" json:"window_size,omitempty" jsonschema:"title=Window Size,description=Minimum guaranteed messages (buffer floor). Dynamically reduced if Budget is too small.,minimum=1,default=20"`
	...
	// Default: 8000
	Budget int `yaml:"budget,omitempty" json:"budget,omitempty" jsonschema:"title=Token Budget,description=Primary token constraint. Dynamically limits WindowSize if set too low.,minimum=1,default=8000"`
...
func (c *StructuredOutputConfig) Validate() error {
	...
	if c.IsStrict() {
		return validateStrictSchema(c.Schema, "schema")
	}

	return nil
}

// validateStrictSchema recursively checks if the schema adheres to strict mode rules.
// Rules:
// 1. additionalProperties must be false for objects.
// 2. All properties must be listed in required.
// 3. Nested objects must also follow these rules.
func validateStrictSchema(schema map[string]interface{}, path string) error {
	typeVal, _ := schema["type"].(string)

	// Only validate objects (or things with properties)
	if typeVal == "object" || schema["properties"] != nil {
		// Rule 1: additionalProperties: false
		ap, ok := schema["additionalProperties"].(bool)
		if !ok || ap {
			return fmt.Errorf("%s: additionalProperties must be false in strict mode", joinPath(path, "additionalProperties"))
		}

		// Rule 2: all properties must be required
		props, ok := schema["properties"].(map[string]interface{})
		if ok {
			reqs, _ := schema["required"].([]interface{})
			...
			for key, val := range props {
				if !reqSet[key] {
					return fmt.Errorf("%s: property %q must be listed in required array in strict mode", path, key)
				}

				// Rule 3: Recurse
				if nested, ok := val.(map[string]interface{}); ok {
					if err := validateStrictSchema(nested, joinPath(path, key)); err != nil {
						return err
					}
				}
			}
...
func isWorkflowAgent(agentType string) bool {
	switch agentType {
	case "sequential", "parallel", "loop", "runner", "conditional":
		return true
	default:
		return false
	}
