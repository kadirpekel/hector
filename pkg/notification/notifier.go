...
func (n *Notifier) sendNotification(ctx context.Context, cfg *config.NotificationConfig, event Event) {
	...
	if err != nil {
		n.metrics.RecordNotificationSent(event.AgentName, string(event.Type), "error", duration)
	...
	if resp.StatusCode >= 400 {
		n.metrics.RecordNotificationSent(event.AgentName, string(event.Type), "failed_status", duration)
	...
	n.metrics.RecordNotificationSent(event.AgentName, string(event.Type), "success", duration)
