...
// ============================================================================
//
// This replaces the old parser.go with a clean, declarative approach.
// Benefits:
...
type CallCmd struct {
	...
	// Positional arguments (message first since it's required, agent optional)
	Message string `arg:"" help:"Message to send to the agent" placeholder:"MESSAGE"`
	Agent   string `arg:"" optional:"" help:"Agent name (optional in zero-config/single-agent mode, default: assistant)" placeholder:"AGENT"`
}
...
type ChatCmd struct {
	...
	// Positional argument
	Agent string `arg:"" optional:"" help:"Agent name (optional in zero-config/single-agent mode)" placeholder:"AGENT"`
}
...
func (s *ServeCmd) ToCLIArgs() *CLIArgs {
	return &CLIArgs{
		...
		VectorDB:                s.VectorDB,
		AgentID:                 s.AgentName,
		ExplicitZeroConfigFlags: s.hasZeroConfigFlags(),
	}
}

// ToCLIArgs converts ListCmd to legacy CLIArgs format
func (l *ListCmd) ToCLIArgs() *CLIArgs {
	return &CLIArgs{
		Command:    CommandList,
		ConfigFile: CLI.Config,
		Debug:      CLI.Debug,
...
// ToCLIArgs converts InfoCmd to legacy CLIArgs format
func (i *InfoCmd) ToCLIArgs() *CLIArgs {
	return &CLIArgs{
		Command:    CommandInfo,
		ConfigFile: CLI.Config,
		Debug:      CLI.Debug,
...
// ToCLIArgs converts CallCmd to legacy CLIArgs format
func (c *CallCmd) ToCLIArgs() *CLIArgs {
	agentID := c.Agent
	if agentID == "" {
		// Zero-config mode: use default agent name
	...
	return &CLIArgs{
		...
		AgentID:                 agentID,
		Input:                   c.Message,
		ExplicitZeroConfigFlags: c.hasZeroConfigFlags(),
	}
}

// ToCLIArgs converts ChatCmd to legacy CLIArgs format
func (c *ChatCmd) ToCLIArgs() *CLIArgs {
	agentID := c.Agent
	if agentID == "" {
		// Zero-config mode: use default agent name
		agentID = config.DefaultAgentName
	...
	return &CLIArgs{
		...
		ServerURL:               c.Server,
		Token:                   c.Token,
		Stream:                  !c.NoStream, // Invert because Kong uses --no-stream
		SessionID:               c.SessionID,
		Provider:                c.Provider,
		Model:                   c.Model,
		APIKey:                  c.APIKey,
...
// ToCLIArgs converts TaskGetCmd to legacy CLIArgs format
func (t *TaskGetCmd) ToCLIArgs() *CLIArgs {
	return &CLIArgs{
		Command:    CommandTask,
		ConfigFile: CLI.Config,
		Debug:      CLI.Debug,
		ServerURL:  CLI.Task.Server,
		Token:      CLI.Task.Token,
		AgentID:    t.Agent,
		TaskID:     t.TaskID,
		TaskAction: "get",
	}
...
// ToCLIArgs converts TaskCancelCmd to legacy CLIArgs format
func (t *TaskCancelCmd) ToCLIArgs() *CLIArgs {
	return &CLIArgs{
		Command:    CommandTask,
		ConfigFile: CLI.Config,
		Debug:      CLI.Debug,
...
// Helper methods to detect zero-config flags
func (s *ServeCmd) hasZeroConfigFlags() bool {
	return s.Provider != "" || s.Model != "" || s.APIKey != "" ||
		s.BaseURL != "" || s.Tools || s.MCPURL != "" || s.DocsFolder != ""
}

func (c *CallCmd) hasZeroConfigFlags() bool {
	return c.Provider != "" || c.Model != "" || c.APIKey != "" ||
		c.BaseURL != "" || c.Tools || c.MCPURL != "" || c.DocsFolder != ""
}

func (c *ChatCmd) hasZeroConfigFlags() bool {
	return c.Provider != "" || c.Model != "" || c.APIKey != "" ||
		c.BaseURL != "" || c.Tools || c.MCPURL != "" || c.DocsFolder != ""
}
