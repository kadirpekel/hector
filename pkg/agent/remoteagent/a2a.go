	...
	return func(yield func(*agent.Event, error) bool) {
		...
		if useStreaming {
			...
			case *a2a.Message:
				...
				event.Artifact = &a2a.Artifact{Parts: r.Parts}
				yield(event, nil)
			}
		}
	}
...
func (a *a2aAgent) errorEvent(ctx agent.InvocationContext, err error) *agent.Event {
	event := a.newEvent(ctx)
	// Create error artifact so it's displayed to the user
	event.Artifact = &a2a.Artifact{
		Parts: []a2a.Part{a2a.TextPart{Text: "Remote agent error: " + err.Error()}},
	}
...
func (a *a2aAgent) convertEvent(ctx agent.InvocationContext, a2aEvent a2a.Event) *agent.Event {
	...
	case *a2a.TaskStatusUpdateEvent:
		...
		if e.Status.Message != nil {
			event.Artifact = &a2a.Artifact{Parts: e.Status.Message.Parts}
		}
		event.Partial = e.Status.State == a2a.TaskStateWorking

	case *a2a.TaskArtifactUpdateEvent:
		// Artifact updates - use artifact directly
		if e.Artifact != nil && len(e.Artifact.Parts) > 0 {
			event.Artifact = e.Artifact
		}
	...
	case *a2a.Task:
		...
		// Only set _hector_task_status if we don't have content
		// (to avoid triggering status-only event path)
		if !hasContent {
			event.Metadata["_hector_task_status"] = e.Status.State
		}
...
func (a *a2aAgent) propagateMetadata(source map[string]any, target *agent.Event) {
	...
	if target.Metadata == nil {
		target.Metadata = make(map[string]any, len(source))
	}
