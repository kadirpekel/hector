...
const (
	...
	TypeConditionalAgent AgentType = "conditional"
)
...
func (a *baseAgent) runBeforeCallbacks(ctx InvocationContext) (*Event, error) {
	...
	for _, cb := range a.beforeAgentCallbacks {
		...
		if msg != nil {
			...
			event.Artifact = &a2a.Artifact{Parts: msg.Parts}
			event.Author = a.displayName
			event.AgentID = a.name
			event.Branch = ctx.Branch()
			event.Actions = *cbCtx.actions
			// Propagate intervention metadata from callback (guardrails, moderation)
			event.SetCustomMetadata(cbCtx.Metadata())
			ctx.EndInvocation()
			return event, nil
		}
	}
...
func (a *baseAgent) runAfterCallbacks(ctx InvocationContext) (*Event, error) {
	...
	for _, cb := range a.afterAgentCallbacks {
		...
		if msg != nil {
			...
			event.Branch = ctx.Branch()
			event.Actions = *cbCtx.actions
			// Propagate intervention metadata from callback
			event.SetCustomMetadata(cbCtx.Metadata())
			return event, nil
		}
	}
