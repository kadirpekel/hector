...
func (s *SummarizationService) SummarizeConversation(ctx context.Context, messages []llms.Message) (string, error) {
	if len(messages) == 0 {
		return "", nil
	}
...
Provide a comprehensive summary that preserves all important context:`, conversationText)
	...
	// Generate summary
	text, _, _, err := s.llm.Generate([]llms.Message{
...
func (s *SummarizationService) SummarizeConversationChunked(ctx context.Context, messages []llms.Message, chunkSize int) (string, error) {
	if len(messages) == 0 {
		return "", nil
	}
...
func (s *SummarizationService) EstimateTokenSavings(messages []llms.Message, summary string) int {
...
func (s *SummarizationService) ShouldSummarize(messages []llms.Message, maxTokens int, threshold float64) bool {
	if len(messages) == 0 {
		return false
	}
...
func (s *SummarizationService) formatConversation(messages []llms.Message) string {
...
type SummarizedHistory struct {
	...
	RecentMessages []llms.Message // Recent messages (kept intact)
}
...
func (sh *SummarizedHistory) ToMessages() []llms.Message {
	if sh.Summary == "" {
	...
	result := make([]llms.Message, 0, 1+len(sh.RecentMessages))
