...
type llmAgent struct {
	...
	// WorkingMemory strategy for context window management
	workingMemory memory.WorkingMemoryStrategy
...
func New(cfg Config) (agent.Agent, error) {
	...
	a := &llmAgent{
		...
		outputSchemaName:          cfg.OutputSchemaName,
		outputSchemaStrict:        cfg.OutputSchemaStrict,
		reasoning:                 reasoning,
		workingMemory:             cfg.WorkingMemory,
		autoRecall:                cfg.AutoRecall,
		autoRecallLimit:           cfg.AutoRecallLimit,
...
func (a *llmAgent) buildMessages(ctx agent.InvocationContext) []*a2a.Message {
	...
	// 4b. Automatic Memory Recall (Legacy Feature)
	// If enabled, we search for long-term memories using the last user message
	// and inject them into the context.
	if a.autoRecall && ctx.Memory() != nil {
		// Find last user message content to use as query
		var query string
		for i := len(events) - 1; i >= 0; i-- {
			if events[i].Author == agent.AuthorUser && events[i].Artifact != nil {
				query = events[i].TextContent()
				break
			}
		}

		if query != "" {
			limit := a.autoRecallLimit
			if limit <= 0 {
				limit = 5
			}

			// Perform search
			results, err := ctx.Memory().Search(ctx, query)
			if err != nil {
				slog.Warn("Auto-recall failed", "error", err)
			} else if results != nil && len(results.Results) > 0 {
				slog.Debug("Auto-recalled memories", "count", len(results.Results))

				// Format memories as a system message or context block
				// We prepend them to the message list as a specialized context message
				// This aligns with how RAG context is typically handled
				memoryContext := buildMemoryContextMessage(results.Results, limit)
				if memoryContext != nil {
					// Prepend memory context to messages list
					// Note: We'll append this to our messages slice later, effectively putting it
					// before the conversation history but after system prompt (if any)
					// Wait, buildMessages returns []*a2a.Message.
					// We should insert it at the beginning of the returned slice.

					// To correctly insert, we need to handle it after this loop.
					// Let's add it to a separate slice to prepend later.
...
func reconstructMessageWithThinking(event *agent.Event) *a2a.Message {
	...
	// If no thinking metadata, return message with original parts
	if event.Metadata == nil {
		return a2a.NewMessage(role, parts...)
	}

	thinkingContent, hasThinking := event.Metadata["thinking"].(string)
	thinkingSignature, hasSignature := event.Metadata["thinking_signature"].(string)
	...
	// Only reconstruct thinking for agent messages
	if event.Author == agent.AuthorUser {
		return a2a.NewMessage(role, parts...)
	}
...
func (a *llmAgent) findTool(ctx agent.InvocationContext, name string) tool.Tool {
	...
	for _, ts := range a.toolsets {
		tools, err := ts.Tools(ctx)
		if err != nil {
			slog.Warn("Toolset error during tool lookup",
				"toolset", ts.Name(),
				"tool_name", name,
				"error", err)
			continue
