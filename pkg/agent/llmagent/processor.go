...
func ConfigRequestProcessor(ctx ProcessorContext, req *model.Request) error {
	...
	if a.outputSchema != nil {
		...
		if a.outputSchemaName != "" {
			req.Config.ResponseSchemaName = a.outputSchemaName
		}

		if a.outputSchemaStrict != nil {
			req.Config.ResponseSchemaStrict = a.outputSchemaStrict
		}
	}
...
func InstructionRequestProcessor(ctx ProcessorContext, req *model.Request) error {
	...
	// Mechanical Safety Protocols (Tool Reliability)
	// Only inject if tools are present and protocols are not disabled
	if !a.disableSafetyProtocols {
		defs, err := ctx.ToolDefinitions()
		if err == nil && len(defs) > 0 {
			// Tool Reliability Protocol
			// Prevents premature completion and hallucination of tool results
			parts = append(parts, "Important: When you call a tool, wait for the actual result before proceeding. Never assume or hallucinate what a tool returned.")
		}
	}
...
func (p *ContentProcessor) listFunctionResponsesFromEvent(e *agent.Event) []FunctionResponseInfo {
	...
	// Check ToolResults from metadata
...
func (p *ContentProcessor) mergeFunctionResponseEvents(events []*agent.Event) *agent.Event {
	...
	// Collect tool results into a local slice
	toolResults := append([]agent.ToolResultState{}, events[0].ToolResults()...)
	...
	// Store merged tool results
	merged.SetToolResults(toolResults)
