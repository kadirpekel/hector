...
func (f *Flow) buildModelResponseEvent(ctx agent.InvocationContext, resp *model.Response, stateDelta map[string]any) *agent.Event {
	...
	if resp.Content != nil {
		event.Artifact = &a2a.Artifact{Parts: resp.Content.Parts}
	}

	// Add tool calls to event
	if len(resp.ToolCalls) > 0 {
		var parts []a2a.Part
		var toolCalls []agent.ToolCallState
		for _, tc := range resp.ToolCalls {
			toolCalls = append(toolCalls, agent.ToolCallState{
				ID:     tc.ID,
				Name:   tc.Name,
				Args:   tc.Args,
				Status: "working",
	...
	// Add thinking state and save to Metadata for multi-turn reconstruction
	...
	if resp.Thinking != nil && resp.Thinking.Content != "" {
		...
		event.SetThinking(&agent.ThinkingState{
			ID:        thinkingID,
			Status:    "completed",
		...
		// Save additional thinking fields to Metadata for multi-turn thinking reconstruction
		// Note: SetThinking already stores the thinking state in metadata
		if resp.Thinking.Signature != "" {
			event.Metadata["thinking_signature"] = resp.Thinking.Signature
		}
	}
	...
	// Propagate response metadata (from guardrails, callbacks) to event
...
func (f *Flow) buildPartialEvent(ctx agent.InvocationContext, resp *model.Response) *agent.Event {
	...
	// Set tool calls if any were collected
	if len(toolCalls) > 0 {
		event.SetToolCalls(toolCalls)
	}

	// Create artifact with all parts
	if len(parts) > 0 {
		event.Artifact = &a2a.Artifact{Parts: parts}
	}
...
func (f *Flow) handleToolCalls(ctx agent.InvocationContext, resp *model.Response, yield func(*agent.Event, error) bool) (*agent.Event, error) {
		...
		if t == nil {
			...
			if approvalDecision == "approve" {
				...
				func() {
					...
					slog.Info("Tool approved, executing", "tool", tc.Name, "callID", tc.ID)
				...
				slog.Info("Tool requires approval", "tool", tc.Name, "callID", tc.ID)
	...
	event.Branch = ctx.Branch()
	event.Partial = false
	event.SetToolResults(toolResults)
	event.Artifact = &a2a.Artifact{Parts: toolResultParts}
...
func (f *Flow) executeStreamingTool(
	...
	for result, err := range st.CallStreaming(toolCtx, tc.Args) {
		...
		if result.Streaming {
			...
			event.SetToolResults([]agent.ToolResultState{{
				ToolCallID: tc.ID,
				Content:    accumulated,
				Status:     "working",
				IsError:    false,
...
func (f *Flow) preparePendingDenialMessages(ctx agent.InvocationContext) {
	...
	for i := events.Len() - 1; i >= 0; i-- {
		event := events.At(i)
		if event == nil || event.Artifact == nil || event.Author == agent.AuthorUser {
			continue
		}
...
func (f *Flow) executePendingApprovedTools(ctx agent.InvocationContext, yield func(*agent.Event, error) bool) bool {
	...
	for _, pt := range pendingTools {
		...
		// Add tool result to artifact
		event.Artifact = &a2a.Artifact{
			Parts: []a2a.Part{a2a.DataPart{
				Data: map[string]any{
					"type":              "tool_result",
...
func (f *Flow) hasFinalToolResult(events agent.Events, fromIndex int, toolCallID string) bool {
	for j := fromIndex + 1; j < events.Len(); j++ {
		...
		for _, part := range event.Parts() {
			...
			if toolResults := event.ToolResults(); toolResults != nil {
