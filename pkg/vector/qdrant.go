...
// Note: Requires additional API call, returns 0 for performance.
func (p *QdrantProvider) Count(ctx context.Context, collection string) (int, error) {
	// Qdrant count would require collection info API call
	// For now, return 0 - this is used for search tool description only
	return 0, nil
}

// DeleteApp cleans up app-specific resources.
func (p *QdrantProvider) DeleteApp(ctx context.Context, appID string) error {
	// TODO: Implement app-specific cleanup for Qdrant
	slog.Warn("DeleteApp not implemented for Qdrant provider - manual cleanup may be required", "app", appID)
	return nil
}
