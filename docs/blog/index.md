---
title: Blog
description: Tutorials, guides, and insights on building production-grade AI agent systems
---

# Hector Blog

Welcome to the Hector blog! Here you'll find tutorials, guides, and insights on building production-grade AI agent systems with Hector.

## Latest Posts

### [Building Enterprise RAG Systems with Hector](posts/building-enterprise-rag-systems.md)

**Published:** November 16, 2025  
**Tags:** Enterprise AI, RAG, On-Premise Deployment, Multi-Source Search

Learn how to build a production-grade RAG system step-by-step that works entirely on-premise, searches across multiple data sources, and provides enterprise observability—all configured in YAML.

[Read more →](posts/building-enterprise-rag-systems.md)

---

### [Building a Cursor-like AI Coding Assistant](posts/building-a-coding-assistant.md)

**Published:** January 16, 2025  
**Tags:** Coding Assistant, Semantic Search, Tools, Tutorial

Create a production-ready coding assistant with semantic code search, intelligent reasoning, and powerful file operations—all through declarative YAML configuration. No code required.

[Read more →](posts/building-a-coding-assistant.md)

---

### [Building a Multi-Agent Research System](posts/building-a-multi-agent-research-system.md)

**Published:** January 16, 2025  
**Tags:** Multi-Agent, Research, Orchestration, Tutorial

Build an automated research assistant that coordinates multiple specialized agents to research topics, analyze findings, and write comprehensive reports.

[Read more →](posts/building-a-multi-agent-research-system.md)

---

### [Adding Custom Tools with MCP](posts/adding-custom-tools-with-mcp.md)

**Published:** January 16, 2025  
**Tags:** Tools, MCP, Extensibility, Tutorial

Extend your agents with custom tools using the Model Context Protocol (MCP). Build a custom tool in minutes, not hours.

[Read more →](posts/adding-custom-tools-with-mcp.md)

---

### [Quick Observability Setup](posts/quick-observability-setup.md)

**Published:** January 16, 2025  
**Tags:** Observability, Monitoring, Metrics, Tracing

Enable monitoring and tracing in development with a single flag - no config file needed!

[Read more →](posts/quick-observability-setup.md)

---

## Categories

### Enterprise AI
- [Building Enterprise RAG Systems](posts/building-enterprise-rag-systems.md)
- [Building a Multi-Agent Research System](posts/building-a-multi-agent-research-system.md)

### RAG & Search
- [Building Enterprise RAG Systems](posts/building-enterprise-rag-systems.md)
- [Building a Cursor-like AI Coding Assistant](posts/building-a-coding-assistant.md)

### Tools & Extensibility
- [Building a Cursor-like AI Coding Assistant](posts/building-a-coding-assistant.md)
- [Adding Custom Tools with MCP](posts/adding-custom-tools-with-mcp.md)

### Multi-Agent Systems
- [Building a Multi-Agent Research System](posts/building-a-multi-agent-research-system.md)

### Observability
- [Quick Observability Setup](posts/quick-observability-setup.md)

### On-Premise Deployment
- [Building Enterprise RAG Systems](posts/building-enterprise-rag-systems.md)

---

## Subscribe

Stay updated with the latest tutorials and guides. Follow us on [GitHub](https://github.com/kadirpekel/hector) or check back regularly for new posts.

