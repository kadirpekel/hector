...
func NewSearchEngine(db providers.DatabaseProvider, embedder providers.EmbedderProvider, searchConfig config.SearchConfig) (*SearchEngine, error) {
	if db == nil {
	...
	if err := engine.validateHealth(context.Background()); err != nil {
...
// validateHealth validates the search engine health
func (se *SearchEngine) validateHealth(ctx context.Context) error {
	// Test embedder
	if _, err := se.embedder.Embed("test"); err != nil {
		return NewSearchError("SearchEngine", "validateHealth", "embedder health check failed", "", err)
	}

	// Test database (if it has a health check method)
	// Note: This would depend on the specific database provider implementation
...
func (se *SearchEngine) IngestDocument(ctx context.Context, docID, content string, metadata map[string]interface{}) error {
	...
	if !se.healthy {
		return NewSearchError("SearchEngine", "IngestDocument", "search engine is not healthy", "", nil)
	}
...
func (se *SearchEngine) SearchModels(ctx context.Context, query string, topKPerModel int, modelNames ...string) (map[string][]providers.SearchResult, error) {
	se.mu.RLock()
	defer se.mu.RUnlock()

	if !se.healthy {
		return nil, NewSearchError("SearchEngine", "SearchModels", "search engine is not healthy", query, nil)
	}
	...
	// Search each model
	results := make(map[string][]providers.SearchResult, len(modelsToSearch))
...
func (se *SearchEngine) Search(ctx context.Context, query string, limit int) ([]providers.SearchResult, error) {
	// Validate limit
	if limit <= 0 {
		limit = DefaultTopK
	}
...
func (se *SearchEngine) searchModel(ctx context.Context, modelName string, vector []float32, topK int) ([]providers.SearchResult, error) {
	...
	convertedResults := make([]providers.SearchResult, len(modelResults))
	for i, result := range modelResults {
		convertedResults[i] = providers.SearchResult{
			ID:        result.ID,
			Score:     result.Score,
...
func (se *SearchEngine) flattenResults(results map[string][]providers.SearchResult, limit int) []providers.SearchResult {
	flatResults := make([]providers.SearchResult, 0, limit)
...
// IsHealthy returns the health status of the search engine
func (se *SearchEngine) IsHealthy() bool {
	se.mu.RLock()
	defer se.mu.RUnlock()
	return se.healthy
}
...
// SetHealthy sets the health status (for testing or manual control)
func (se *SearchEngine) SetHealthy(healthy bool) {
	se.mu.Lock()
	defer se.mu.Unlock()
	se.healthy = healthy
}
