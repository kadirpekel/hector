...
func NewQdrantDatabaseProvider() (providers.DatabaseProvider, error) {
...
func NewQdrantDatabaseProviderFromConfig(config *config.DatabaseProviderConfig) (providers.DatabaseProvider, error) {
	config.SetDefaults()
...
func (db *qdrantDatabaseProvider) Search(ctx context.Context, collection string, queryVector []float32, topK int) ([]providers.SearchResult, error) {
...
// IsHealthy checks if the Qdrant database is healthy and ready to use
func (db *qdrantDatabaseProvider) IsHealthy(ctx context.Context) bool {
	// Simple health check - try to check if a collection exists
	// This is a lightweight operation that tests basic connectivity
	_, err := db.client.CollectionExists(ctx, "health_check")
	return err == nil
}
