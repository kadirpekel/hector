...
import { Shield, Check, X, ChevronDown, ChevronRight, Loader2 } from 'lucide-react';
...
export const ApprovalWidget: React.FC<ApprovalWidgetProps> = ({ widget, sessionId, onExpansionChange, shouldAnimate = false }) => {
    ...
    const handleDecision = async (decisionValue: 'approve' | 'deny') => {
        ...
        try {
            ...
            const approvalMessage = session.messages.find(m => m.widgets.some(w => w.id === widget.id));
            if (!approvalMessage) {
        ...
        <div className="border border-white/10 rounded-lg bg-black/20 overflow-hidden text-sm">
            ...
            >
                <Shield 
                    size={14} 
                    className={cn(
                        status === 'pending' ? "text-yellow-400" : decision === 'approve' ? "text-green-400" : "text-red-400",
                        shouldAnimate && "animate-[badgeLifecycle_2s_ease-in-out_infinite]"
                    )}
                />
                <span className={cn(
                    "font-medium",
                    status === 'pending' ? "text-yellow-200" : decision === 'approve' ? "text-green-200" : "text-red-200"
                )}>
