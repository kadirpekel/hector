...
const SessionList = React.memo(({ sessions, currentSessionId, selectSession, deleteSession }: SessionListProps) => {
    // derived state
    const sortedSessions = React.useMemo(() => Object.values(sessions).sort(
        (a: SessionMeta, b: SessionMeta) => new Date(b.created).getTime() - new Date(a.created).getTime()
    ), [sessions]);
